...
const AboutTechnologiesText = () => {
  ...
  return (
    <motion.figcaption
      className={`w-full h-1/2 flex flex-col gap-5 ${montserrat.className}`}
      ref={ref}
      variants={scrollVariant}
      animate={animation}
      initial="hidden"
    >
      <h2 className={`text-center sm:text-left self-center sm:self-start font-bold text-2xl sm:text-2xl ${ubuntu.className}`}>
        Get More Info!
      </h2>
      <div className="w-[85%] self-center md:self-start">
        {aboutInfoText.map((item, index) => (
          <details
            ...
            key={index}
          >
            <summary className="flex items-center gap-1 marker:content-none hover:cursor-pointer justify-between font-semibold hover:text-scheme-greenOne">
              {item.title}
